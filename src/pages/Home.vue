      ...
      <div class="front-sidemenu">
        <ul>
          <li
            ...
            @click="active = index"
          >
            <div @click="getItems(categories.attributes.title)">
      ...
      <div class="main-grid">
        <div class="menu-section">
          ...
          <div class="menu-grid" v-if="selectedMenu">
            <div
              :style="`background-image: url(${item.attributes.url})`"
              class="menu-card"
                ...
                >
                  <div class="cart-info">
                    ...
                    <div>
                      ...
                      <small
                        ...
                        {{
                          (
                            Number(item.attributes.price) * Number(item.qty)
                          ).toLocaleString()
                        }}</small
                      >
                    </div>
                  ...
                  <div class="cart-controls">
                    ...
                    <div>
                      <span
                        class="ti-angle-up"
                        @click="updateItemQty(index, item, 1)"
                      ></span>
                      <span
                        class="ti-angle-down"
                        @click="updateItemQty(index, item, 0)"
                      ></span>
                    </div>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    getItems(role) {
      this.filterFoot = role;
      this.getProduct();
    },
    getProduct() {
      this.$axios
        .get(`${this.$apiUrl}products/?populate=*`)
        .then((res) => {
          if (this.filterFoot) {
            this.product = res?.data?.data.filter((item) => {
              return item.attributes.role === this.filterFoot;
            });
          } else {
            this.product = res?.data?.data.filter((item) => {
              return item.attributes.role === "foods";
            });
          }
        })
        .catch((error) => {
    ...
    getCategories() {
      this.$axios
        .get(`${this.$apiUrl}categories/?populate=*`)
        .then((res) => {
    ...
    createOrder(response) {
      ...
      this.$axios
        .post(`${this.$apiUrl}/create-orders`, data, {
          headers: {
  ...
  computed: {
    getRoleTitle() {
      const computedAttribute = this.product.find(
        (attribute) => attribute.attributes.role
      );
      return computedAttribute ? computedAttribute.attributes.role : "";
    },
    user() {
    ...
    cartTotal() {
      let total = 0;
      this.cart.map((item) => {
        total += Number(item.attributes.price) * Number(item.qty);
      });
